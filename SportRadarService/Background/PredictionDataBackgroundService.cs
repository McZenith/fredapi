...
// Modified UpdatePredictionDataAsync method in PredictionDataBackgroundService.cs
    private async Task UpdatePredictionDataAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            // Use direct pagination instead of time ranges
            ...
            for (int page = 0; page < pageCount; page++)
            {
                try
                {
                    ...
                    _logger.LogInformation(
                        $"Fetching page {page + 1}/{pageCount} (skip: {skipCount}, limit: {PAGE_SIZE})");
            ...
            _logger.LogInformation(
                $"Successfully retrieved {allMatches.Count} matches out of {totalMatchCount} total matches");
            ...
            _logger.LogInformation(
                $"Successfully transformed {enrichedMatches.Count} out of {allMatches.Count} matches");
            ...
            foreach (var batch in transformBatches)
            {
                ...
                var task = Task.Run(async () =>
                {
                    ...
                    try
                    {
                        ...
                        var batchResult = await Task.Run(() => transformer.TransformToPredictionData(batch.ToList()),
                            linkedCts.Token);
                        ...
                        _logger.LogInformation(
                            $"Transformed batch {completedBatches}/{totalBatches} with {batch.Length} matches");
                    }
                    catch (OperationCanceledException)
                    {
            ...
            _logger.LogInformation(
                $"Successfully transformed {allMatchesList.Count} matches out of {enrichedMatches.Count} enriched matches");
            ...
            // IMPORTANT CHANGE: Create the full response object to cache
            var fullResponse = new PredictionDataResponse
            {
            ...
            // Cache the FORMATTED response object, not just the raw matches
            _cache.Set(CACHE_KEY_PREDICTION_DATA, fullResponse, TimeSpan.FromHours(2));

            // Now send the data to clients, with pagination if needed
            const int MAX_MATCHES_PER_RESPONSE = 200;
            int totalPages = (int)Math.Ceiling(allMatchesList.Count / (double)MAX_MATCHES_PER_RESPONSE);

            if (totalPages <= 1)
            {
                // If it's small enough, send it as one page
                await hubContext.Clients.All.SendAsync("ReceivePredictionData", fullResponse, stoppingToken);
                _logger.LogInformation($"Sent all {allMatchesList.Count} matches in a single response to clients");
            }
            else
            {
                // Otherwise, paginate for sending
                for (int page = 0; page < totalPages; page++)
                {
                    var skip = page * MAX_MATCHES_PER_RESPONSE;
                    var take = Math.Min(MAX_MATCHES_PER_RESPONSE, allMatchesList.Count - skip);
                    ...
                    _logger.LogInformation(
                        $"Sent page {page + 1}/{totalPages} with {pageMatches.Count} matches to clients");
            ...
            _logger.LogInformation($"Successfully updated prediction data for {allMatchesList.Count} matches");
        }
        catch (Exception ex)
        {
