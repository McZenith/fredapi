    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    _logger.LogInformation($"Skipping prediction results processing. Next run in {(_resultProcessingInterval - timeElapsedSinceLastProcessing).TotalMinutes:F1} minutes");
                }
            }
            catch (Exception ex)
            {
